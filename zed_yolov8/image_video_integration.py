...
overlay_image_path = "replacement_images/rep_zoomed_person_135.jpg"
...
# Function to overlay image on bounding box
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            if closest_person_idx != -1:
                ...
                cv2.imwrite(zoomed_in_filename, zoomed_in_image)
                print(f"Saved zoomed-in image of person {closest_person_idx + 1} as {zoomed_in_filename}")
                
                overlay_on_bounding_box(image_net, x_min, y_min, x_max, y_max, overlay_image)
            else:
